...
// В этом примере класс DivisionCalculator содержит метод divideNumbers(), который выполняет деление двух чисел и выбрасывает исключение DivisionByZeroException при делении на ноль. В методе main() мы используем этот метод для ввода чисел с клавиатуры, выполнения деления и обработки возможных исключений.

// В случае некорректного ввода чисел, будет перехвачено исключение NumberFormatException и выведено соответствующее сообщение об ошибке. Если возникнет исключение DivisionByZeroException, оно будет перехвачено в блоке catch и обработано методом handleDivisionByZeroException(). В этом методе можно выполнить дополнительные действия по обработке исключения на более высоком уровне абстракции.
